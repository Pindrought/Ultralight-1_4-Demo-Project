...
void CustomGPUImpl::BeginSynchronize()
{
    ...
    for (auto viewEntry : pManager->GetViews())
    {
        ...
        uint32_t view_id = nativeView->display_id();
        auto rt = nativeView->render_target();

        auto iter = m_ViewToTextureIdMap.find(view_id);
        if (iter == m_ViewToTextureIdMap.end())
        {
            m_ViewToTextureIdMap[view_id] = rt.texture_id;
        }
        else
        {
            if (iter->second != rt.texture_id)
            {
                m_ViewToTextureIdMap[view_id] = rt.texture_id;
            }
        }
...
uint32_t CustomGPUImpl::NextTextureId()
{
    return s_UltralightGPUDriverTextureIDManager.GetNextId();
}

uint32_t CustomGPUImpl::NextRenderBufferId()
{
    return s_UltralightGPUDriverRenderBufferIDManager.GetNextId();
}

uint32_t CustomGPUImpl::NextGeometryId()
{
    return s_UltralightGPUDriverGeometryIDManager.GetNextId();
}

void CustomGPUImpl::UpdateCommandList(const ul::CommandList& list)
{
    if (list.size)
    {
...
void CustomGPUImpl::DrawCommandList()
{
...
CustomGPUImpl::CustomGPUImpl()
{
    ...
    {
        FatalError("GPUDriverD3D1::CustomGPUImpl failed to initialize constant buffer.");
    }
}

void CustomGPUImpl::CreateTexture(uint32_t textureId, ul::RefPtr<ul::Bitmap> bitmap)
{
    auto i = m_TextureMap.find(textureId);
    if (i != m_TextureMap.end())
        FatalError("CustomGPUImpl::CreateTexture, texture id already exists.");

    if (!(bitmap->format() == ul::BitmapFormat::BGRA8_UNORM_SRGB ||
          bitmap->format() == ul::BitmapFormat::A8_UNORM))
        FatalError("CustomGPUImpl::CreateTexture, unsupported format.");

    string msg = strfmt("CreateTexture %d x %d", bitmap->width(), bitmap->height());
    LOGINFO(msg.c_str());

    if (bitmap->width() == 1 && bitmap->height() == 1) //We need to check if this is a staging texture for a CPP texture
    {
        if (bitmap->row_bytes() == 4)
        {
            const void* pPixelData = bitmap->LockPixels();
            uint32_t pixelUID = 0;
            memcpy(&pixelUID, pPixelData, sizeof(uint32_t));

            auto iter = m_TextureOverwriteData.find(pixelUID);
            if (iter == m_TextureOverwriteData.end())
            {
                LOGINFO("Missing texture overwrite data... This is not good..");
            }
            else
            {
    ...
    if (textureEntry.isMSAARenderTarget)
    {
        ...
        FatalErrorIfFail(hr, "CustomGPUImpl::CreateTexture, unable to create MSAA resolve texture.");

        srv_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;

        hr = pd3d->device->CreateShaderResourceView(textureEntry.resolveTexture.Get(), &srv_desc, &textureEntry.resolveSRV);
        FatalErrorIfFail(hr, "CustomGPUImpl::CreateTexture, unable to create shader resource view for MSAA resolve texture.");
    }
#endif
}

void CustomGPUImpl::UpdateTexture(uint32_t textureId, ul::RefPtr<ul::Bitmap> bitmap)
{
    auto iter = m_TextureMap.find(textureId);
    if (iter == m_TextureMap.end())
        FatalError("CustomGPUImpl::UpdateTexture, texture id doesn't exist.");
...
void CustomGPUImpl::DestroyTexture(uint32_t textureId)
{
    ...
    {
        FatalError("CustomGPUImpl::DestroyTexture");
    }
}

void CustomGPUImpl::CreateRenderBuffer(uint32_t renderBufferId_, const ul::RenderBuffer& buffer_)
{

    if (renderBufferId_ == 0)
        FatalError("CustomGPUImpl::CreateRenderBuffer, render buffer ID 0 is reserved for default render target view.");

    auto i = m_RenderTargetMap.find(renderBufferId_);
    if (i != m_RenderTargetMap.end())
        FatalError("CustomGPUImpl::CreateRenderBuffer, render buffer id already exists.");
...
void CustomGPUImpl::DestroyRenderBuffer(uint32_t renderBufferId_)
{
    ...
    {
        FatalError("CustomGPUImpl::DestroyRenderBuffer");
    }
}

void CustomGPUImpl::CreateGeometry(uint32_t geometryId,
...
void CustomGPUImpl::UpdateGeometry(uint32_t geometryId,
                                    const ul::VertexBuffer& vertices,
                                    const ul::IndexBuffer& indices)
{
    auto iter = m_GeometryMap.find(geometryId);
    if (iter == m_GeometryMap.end())
        FatalError("CustomGPUImpl::UpdateGeometry, geometry id doesn't exist.");
...
void CustomGPUImpl::DrawGeometry(uint32_t geometryId,
                                  uint32_t indexCount,
...
void CustomGPUImpl::ClearRenderBuffer(uint32_t renderBufferId)
{
    ...
    if (textureIter == textureMap.end())
        FatalError("CustomGPUImpl::ClearRenderBuffer, render target texture id doesn't exist.");
...
ID3D11ShaderResourceView* CustomGPUImpl::GetShaderResourceView(ul::View* pView)
{
...
void CustomGPUImpl::LoadShaders()
{
    bool result;
...
void CustomGPUImpl::InitializeSamplerState()
{
    ...
    FatalErrorIfFail(hr, "CustomGPUImpl::InitializeSamplerAndBlendStates Failed to create sampler state.");
}

void CustomGPUImpl::InitializeBlendStates()
{
    ...
    FatalErrorIfFail(hr, "CustomGPUImpl::InitializeBlendStates failed to create enabled blend state");
    ...
    FatalErrorIfFail(hr, "CustomGPUImpl::InitializeBlendStates failed to create disabled blend state");
}

void CustomGPUImpl::InitializeRasterizerStates()
{
    HRESULT hr;
    ...
    FatalErrorIfFail(hr, "CustomGPUImpl::InitializeRasterizerStates failed to create default rasterizer state.");
    ...
    FatalErrorIfFail(hr, "CustomGPUImpl::InitializeRasterizerStates failed to create scissored rasterizer state.");
}

void CustomGPUImpl::BindRenderBuffer(uint32_t renderBufferId)
{
    ...
    if (renderTargetTexture == textureMap.end())
        FatalError("CustomGPUImpl::BindRenderBuffer, render target texture id doesn't exist.");
...
void CustomGPUImpl::SetViewport(uint32_t width, uint32_t height)
{
    D3D11_VIEWPORT vp = {};
...
void CustomGPUImpl::BindTexture(uint8_t textureUnit, uint32_t textureId)
{
    auto iter = m_TextureMap.find(textureId);
    if (iter == m_TextureMap.end())
        FatalError("CustomGPUImpl::BindTexture, texture id doesn't exist.");
...
void CustomGPUImpl::UpdateConstantBuffer(const ul::GPUState& state)
{
...
void CustomGPUImpl::BindGeometry(uint32_t geometryId_)
{
    auto iter = m_GeometryMap.find(geometryId_);
    if (iter == m_GeometryMap.end())
        FatalError("CustomGPUImpl::BindGeometry geometry id does not exist in geometry map.");
...
ul::Matrix CustomGPUImpl::ApplyProjection(const ul::Matrix4x4& transform, float screenWidth, float screenHeight)
{
