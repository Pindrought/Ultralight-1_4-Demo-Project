#include "PCH.h"
#include "DemoFileBrowser.h"
#include "../Misc/CursorManager.h"

bool DemoFileBrowser::Startup()
{
	...
	windowParms.Title = "File Browser";
	...
	pView->LoadURL("file:///Samples/FileBrowser/FileBrowser.html");
	m_UltralightMgr.SetViewToWindow(pView->GetId(), pWindow->GetId());
}

bool DemoFileBrowser::Tick()
{
...
EZJSParm DemoFileBrowser::OnEventCallbackFromUltralight(int32_t viewId, string eventName, vector<EZJSParm> parameters)
{
	return EZJSParm();
}

void DemoFileBrowser::OnWindowDestroyCallback(int32_t windowId)
{
...
void DemoFileBrowser::OnWindowResizeCallback(Window* pWindow)
{
}